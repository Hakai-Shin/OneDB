 ...
 * @return Page 
 */
Page BufferManager::getPage(string tableName, int pageIndex, int pageId)
{
    logger.log("BufferManager::getPage");
    string pageName = "../data/temp/"+tableName + "_Page" + to_string(pageId);
    if (this->inPool(pageName))
        return this->getFromPool(pageName);
    else
        return this->insertIntoPool(tableName, pageIndex, pageId);
}
 ...
 * @return Page 
 */
Page BufferManager::insertIntoPool(string tableName, int pageIndex,int pageId)
{
    logger.log("BufferManager::insertIntoPool");
    Page page(tableName, pageIndex, pageId);
    if (this->pages.size() >= BLOCK_COUNT)
        pages.pop_front();
    pages.push_back(page);
    return page;
 ...
 * @param pageIndex 
 */
void BufferManager::deleteFile(string tableName, int pageId)
{
    logger.log("BufferManager::deleteFile");
    string fileName = "../data/temp/"+tableName + "_Page" + to_string(pageId);
 ...
 * @param pageIndex 
 */
void BufferManager::updatePool(string tableName, int pageIndex, vector<vector<int>> rows, int rowCount, string condition)
{
    int targetPagePos = 0;
    logger.log("BufferManager::updatePool");
    string pageName = "../data/temp/"+tableName + "_Page" + to_string(pageIndex);
    if(condition == "delete")
    {
        for (auto page : this->pages)
        {
            if (pageName == page.pageName)
                {
                    pages.erase(pages.begin() + targetPagePos);
                }
            else
            {
                targetPagePos++;
    ...
    {
        Page updatedPage(tableName, pageIndex, rows, rowCount);
        if( condition == "update")
        {
            for (auto page : this->pages)
            {
            ...
            this->pages.push_back(updatedPage);
        }
